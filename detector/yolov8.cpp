...
int32_t Yolov8::Initialize(const std::string& model) {
    ...
    trt_bridge_.reset(TrtBridge::Create(model, p_meta));

    if (!trt_bridge_) {
        return 0;
    }
    if (trt_bridge_->Initialize() != 1) {
    ...
    if (trt_bridge_->GetOutputChannelNum() != kOutputChannelNum) {
        std::cout << "output channel size mismatched" << std::endl;
        return 0;
    }
    for (const auto& grid_scale : kGridScaleList) {
        OutputSpatialSize += (trt_bridge_->GetInputWidth() / grid_scale) * (trt_bridge_->GetInputHeight() / grid_scale);
    }
    OutputSpatialSize *= kElmentAnchorNum;
    if (trt_bridge_->GetOutputLength() != OutputSpatialSize) {
...
int32_t Yolov8::Process(cv::Mat& original_mat, Result& result) {
    ...
    trt_bridge_->PreProcess(original_mat, input_name, kCropStyle::CropAll_Embedd);
    const auto& t_pre_process1 = std::chrono::steady_clock::now();

    // 2. inference
    trt_bridge_->Inference();

    // 3.1 post-process, retrive output and scale bboxes
    const auto& t_post_process0 = std::chrono::steady_clock::now();
    const float* output = trt_bridge_->GetInferenceOutput(output_name);
    cv::Rect dst_crop = trt_bridge_->GetCropInfo().second;
    std::vector<Bbox2D> bbox_list;
    int32_t index = 0;
    for (const auto& grid_scale : kGridScaleList) {
        int32_t grid_w = trt_bridge_->GetInputWidth(input_name) / grid_scale;
        int32_t grid_h = trt_bridge_->GetInputHeight(input_name) / grid_scale;
