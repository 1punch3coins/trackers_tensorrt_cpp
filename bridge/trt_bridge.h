...
class TrtBridge {
public:
    static TrtBridge* Create(const std::string& model_pwd, NetworkMeta* t_info);
    int32_t Initialize();
...
private:
    template <typename T>
    int32_t PermuateAndNormalize(T* intput_ptr, uint8_t* src, int32_t input_h, int32_t input_w, int32_t input_c);
...
private:
    ...
    std::vector<void*> gpu_binding_input_ptrs_;
    std::vector<void*> gpu_binding_output_ptrs_;
